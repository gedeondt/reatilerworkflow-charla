...
type GenerateScenarioJsonSuccess = { ok: true; scenario: ScenarioContract };
type GenerateScenarioJsonFailure = {
  ok: false;
  failure: InspectScenarioContractFailure;
  response: string;
};

type GenerateScenarioJsonResult = GenerateScenarioJsonSuccess | GenerateScenarioJsonFailure;

const buildResponseSnippet = (response: string): string =>
...
const generateScenarioJson = async (
  ...
  const systemMessage: ChatCompletionMessageParam = {
    role: 'system',
    content: `Responde siempre en español. Ajusta la salida al DSL de escenarios definido en este proyecto.\n${scenarioDslRules}`,
  };
  ...
  app.log.warn(
    {
      ...
      responseSnippet: buildResponseSnippet(firstResponse),
    },
    'La primera respuesta del modelo no cumple el contrato del escenario',
  );
  ...
  app.log.warn(
    {
      ...
      responseSnippet: buildResponseSnippet(secondResponse),
    },
    'La segunda respuesta del modelo no cumple el contrato del escenario',
  );
  ...
  try {
    ...
    if (!generationResult.ok) {
      return reply.status(422).send({
        ...
        message: 'El JSON generado no cumple el contrato esperado.',
