...
> => {
  ...
  app.log.warn(
    ...
    'El modelo devolvió un contrato de escenario inválido',
  );
  ...
  app.log.warn(
    ...
    'El modelo devolvió un contrato de escenario inválido tras el reintento',
  );
...
app.post<{ Params: DraftParams; Body: unknown }>('/scenario-drafts/:id/generate-json', async (request, reply) => {
  ...
  try {
    ...
    if (!generationResult.ok) {
      const message =
        generationResult.failure.type === 'invalid_json'
          ? 'La respuesta del modelo no es JSON válido.'
          : 'El escenario generado no cumple el contrato esperado.';
    ...
    return reply.send({
      id: draft.id,
      status: 'generated',
      generatedScenario: scenarioContract,
    });
  } catch (error) {
    if (error instanceof OpenAIRequestFailedError) {
      return reply
